...
\textcolor{brown}{Warning messages: In model.response(mf, "numeric") : using type = "numeric" with a factor response will be ignored. In Ops.factor(y, ...) : '-' not meaningful for factors}
```
...
f_pNum(summary(mod_bb)$coefficients)
#
# #Anova Table 
...
## Data: CarDekho {#set-cardekho-b25}

\textcolor{pink}{Please import the "B25-CarDekho.csv".}

```{r 'B25-CarDekho', include=FALSE, eval=FALSE}
# #Path of Object, FileName and MD5Sum tools::md5sum(paste0(.z$XL, "B25-CarDekho.csv"))
xxB25CarDekho <- f_getObject("xxB25CarDekho", "B25-CarDekho.csv",
                                "92770bca1b81e9339def909673097b97")
```

```{r 'B25-GetCarDekho', include=FALSE}
xxB25CarDekho <- f_getRDS(xxB25CarDekho)
```
...
aa <- xxB25CarDekho
#str(aa)
# #Split String by Space (29 Brands) | To Factor | Calculate Age | Drop |
bb <- aa %>% 
...
# #Dummy
str(bb)
dum_bb <- dummy_cols(bb, select_columns = c("fuel", "seller_type", "transmission", "owner"), 
                     remove_most_frequent_dummy = TRUE, remove_selected_columns = TRUE)
names(dum_bb)
#
mod_bb <- lm(selling_price ~ ., data = bb)
f_pNum(summary(mod_bb)$coefficients)
#
# #Anova Table 
anova(mod_bb) %>% as_tibble(rownames = "Predictors")
...
f_pNum(summary(mod_sal_f)$coefficients)
#
...
f_pNum(summary(mod_sal_m)$coefficients)
#
...
f_pNum(summary(mod_sal_r)$coefficients)
#
