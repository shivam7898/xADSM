...
- ${}^U\!P_{t = 11.295} \approx 0 \to {}^U\!P_{(z)} < {\alpha} \to {H_0}$ is rejected
  - We can conclude that at least 15% of users access D6 related information.
  - \textcolor{orange}{Caution:} Professor applied Proportion. I do not know whether the application of One-tail Test here is wrong or not.
...
```{r 'B-'}
# #Get Percentage Users of D6 
bb <- xxJdata %>% 
  rename(Dates = "Month-Year", Users = "No of users") %>% 
  select(Dates, Users, D6) %>% 
  mutate(across(Dates, as_date)) %>% 
  mutate(PCT = D6/Users)
#
# #One Sample t-Test
t.test(bb$PCT, mu = 0.15, alternative = "greater", conf.level = 0.05)
```
