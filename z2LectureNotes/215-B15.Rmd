...
```{r 'B15-List-A', ref.label=c('B-Jnames', 'B-List')}
#
```
...
#
# #Confirmed Usage is Sum Total of Disease, Variety, Micronutrients
unique(aa$DIFF)
#
...
Also, using data from January-2016 – May-2018, test the claim that app usage picked up after January 2016.
...
- Howeve, only the Week 1 and Week 2 follow Normality, Week 3 and Week 4 data is not normal

```{r 'B-Q4'}
# #Data | Missing Months can be ignored as those are missing across all weeks
bb <- xxJdata %>% 
  rename(Dates = "Month-Year", Users = "No of users") %>% 
  select(Week, Usage)
#
...
#### Normality {.unlisted .unnumbered .tabset .tabset-fade}


```{r 'B-'}
# #Are the data from each of the 4 groups follow a normal distribution
# #Shapiro-Wilk normality test
bb %>% group_by(Week) %>% summarise(N = n(), Mean = mean(Usage), SD = sd(Usage),
                                    p_Shapiro = shapiro.test(Usage)$p.value, 
                                    isNormal = ifelse(p_Shapiro > 0.05, TRUE, FALSE))
# #Week 3 and 4 are not normal
#
# #Check Q-Q plot
#qqnorm(bb[bb$Week == "Week1", ]$Usage)
#
# #Converting the data to normal using log (because the lowest value is 1 in the Usage)
ii <- bb %>% mutate(Log = log(Usage))
ii %>% group_by(Week) %>% summarise(p = shapiro.test(Log)$p.value)
# #Week 1 and 4 are not normal now.
```
