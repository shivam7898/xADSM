...
f_getDef("Supervised-Methods") #dddd
```
...
- Techniques:
  ...
  - Supervised : 
    ...
    - Classification (Categorical Target) : Decision Tree, Random Forest, Logistic Regression, k-nearest neighbors, Bagging and Boosting, Neural Networks
...
```{r 'B16-GetTransaction', include=FALSE}
...
# #and 48 customers were not captured by our classifications and assigned 'Others'
divisions %>% 
  count(segment) %>% 
...
```{r 'B16-GetCustomer', include=FALSE}
# #This is same dataset as rfm_data_orders #wwww
...
```{r 'B19-Segments-A', ref.label=c('B19-Segments'), include=TRUE}
#
```
