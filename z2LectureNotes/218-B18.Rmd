...
- Compare the area code and state fields. Discuss any apparent abnormalities.
  - There are only 3 Area Codes and all of them belonds to California State.
...
if(!file.exists(loc_png)) ggsave(loc_png, plot = B18P03, width = 14)
```
...
if(!file.exists(loc_png)) ggsave(loc_png, plot = B18P04, width = 14)
```
...
```{r 'B-DepCheck01', include=FALSE, eval=FALSE}
...
if(!file.exists(loc_png)) ggsave(loc_png, plot = B18P14, width = 14)
```
...
if(!file.exists(loc_png)) ggsave(loc_png, plot = B18P15, width = 14)
```
...
if(!file.exists(loc_png)) ggsave(loc_png, plot = B18P16, width = 14)
```
