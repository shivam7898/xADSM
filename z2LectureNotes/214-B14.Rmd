...
- Compare with ${\alpha} = 0.05$
  - ${}^U\!P_{(z)} > {\alpha} \to {H_0}$ cannot be rejected 
  - We cannot claim that we win 'at least' 50% leads
...
- ${}^U\!P_{z} < {\alpha} \to {H_0}$ is rejected i.e. the proportions are different
  - We can say, with statistical confidence, that the "learnsys" performs better than "Finsys"
...
- ${}^U\!P_{t} > {\alpha} \to {H_0}$ cannot be rejected 
  - We cannot say that the "learnsys" has higher average sales than "Finsys"
...
```{r 'B-'}
ii <- bb %>% filter(RG %in% c("UK", "India", "Americas")) %>% select(RG, PP)
#
# #ANOVA
