...
bb <- aa %>% 
  ...
  select(State, Data.Population, starts_with("Data.Rates") & !ends_with("All")) %>% 
  select(-State) %>% 
  mutate(across(everything(), ~ as.vector(scale(.))))
...
```{r 'B21P0203', echo=FALSE, ref.label=c('B21P02', 'B21P03'), fig.cap="Crime: Elbow Curve of k Silhouette and WSS"}
knitr::include_graphics(paste0(.z$PX, "B21P01", "-Movie-Elbow-Sil", ".png"))
```
...
res_crime <- cbind(bb, list(k3 = k3_bb$cluster, k4 = k4_bb$cluster)) %>% as_tibble()
#
# #Three Clusters
ii <- k3_bb
ii$size
...
f_rmExist(aa, bb, ii, jj, kk, ll, B21P01, B21P02, B21P03, cap_hh, hh, k3_bb, k4_bb, loc_png, 
          res_crime, ttl_hh, xxB20Movies, xxB21Crime)
```
