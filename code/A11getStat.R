...
f_get2sSE <- function(s1, n1, s2, n2) {
  ...
  cat(paste0("- \\@ref(eq:se-2s-sd) ${\\sigma}_{({\\overline{x}}_1 - {\\overline{x}}_2)} = \\sqrt{\\frac{{", 
  s1, "}^2}{", n1, "} + \\frac{{", s2, "}^2}{", n2, "}} = ", xx,"$\n"))
  cat(paste0("- ", "`", "sqrt(", s1, "^2/", n1, " + ", s2, "^2/", n2, ")", 
  "`", " \\textcolor{pink}{$\\#\\mathcal{R}$}", "\n"))
  return(xx)
}
...
f_get2sDof <- function(s1, n1, s2, n2) {
  ...
  cat(paste0("- ", "`", "floor({", s1,"^2 / ", n1, " + ", s2, "^2 / ", n2, " }^2 / {{", 
...
f_getSeProp <- function(p1, n1, p2, n2) {
  ...
  yy <- {n1 * p1 + n2 * p2} / {n1 + n2}
  cat(paste0("- ", "`", "sqrt(", p1, " * (1 - ", p1, ") / ", n1, " + ", p2, " * (1 - ", p2, 
  ") / ", n2, ")", "`", " \\textcolor{pink}{$\\#\\mathcal{R}$}", "\n"))
  cat(paste0("SE = ", xx, "\n"))
  cat(paste0("- ", "`", "{", n1, " * ", p1, " + ", n2, " * ", p2, "} / {", n1, " + ", n2, "}", 
  "`", " \\textcolor{pink}{$\\#\\mathcal{R}$}", "\n"))
  cat(paste0("p = ", yy, "\n"))
  return(xx)
}
