...
Predicting a qualitative response (i.e. categorical Y) for an observation can be referred to as classifying
...
- \textcolor{pink}{Logistic Function: $p(X) = \frac{e^{\beta_0 + \beta_1 X}}{1 + e^{\beta_0 + \beta_1 X}} = \frac{\text{Odds}}{1 + \text{Odds}}$}
...
#
# #Confusion Matrix
cmat_def_lda <- confusionMatrix(predict(mod_def_lda, type = "class")$class, 
...
#
# #Coefficient Estimates i.e. coefficients of linear discriminants
...
- NOTE: A (TP), B (FP), C (FN), D (TN) counts are different for LDA model and for GLM model
...
prob_glm <- predict(mod_def_glm, type = "response")
if(TRUE) str(prob_glm, give.attr = FALSE)
#
# #Use Contrast to define the criteria
contrasts(zzF67Default$default)
#
# #Convert Probabilities into levels
res_def_glm <- factor(ifelse(prob_glm >= 0.5, "Yes", "No"), levels = c("No", "Yes"))
...
## Confusion Matrix {.tabset .tabset-fade}
...
- Others
  - \textcolor{pink}{False Positive Rate (FPV) or Type-I Error}
...
```{r 'F67T01', echo=FALSE}
# #Print Kable Table
hh <- tibble(Predicted_Reference = c("Event_nonNull_Pos", "noEvent_Null_Neg", "Total"),
             Event_nonNull_Pos = c("TP= A= 79", "FN= C= 254", "A+C= 333"), 
             noEvent_Null_Neg = c("FP= B= 22", "TN= D= 9645", "B+D= 9667"), 
...
cmat <- caret::confusionMatrix(data = predict(mod_def_lda, type = "class")$class, 
                               reference = zzF67Default$default, positive = "Yes")
#
...
#
# #Confusion Matrix
cmat_no <- confusionMatrix(res_def_lda$class, zzF67Default$default, positive = "No")
cmat_yes <- confusionMatrix(res_def_lda$class, zzF67Default$default, positive = "Yes")
#
...
tibble(Names = names(cmat$byClass), Yes = cmat_yes$byClass, No = cmat_no$byClass,
       Match = ifelse(round(abs(Yes - No), 5) < 0.0001, TRUE, ""))
```
...
f_rmExist(aa, bb, ii, jj, kk, ll, cap_hh, cmat, cmat_def_glm, cmat_def_lda, cmat_no, cmat_yes, 
          ...
          mod_def_exp, mod_def_glm, mod_def_imp, mod_def_lda, mod_def_student, prob_glm, 
          res_def_glm, res_def_lda, sub_hh, ttl_hh, x_hh, xfw, y_hh, zzF67Default, 
          zzF67Default_dum, zzF67Smarket)
```

```{r 'F67-Validation', include=TRUE, cache=FALSE}
