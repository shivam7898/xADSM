...
include_graphics(paste0(".", "/images/", "F67P01", "-Default-1x-YX", ".png"))
```
...
include_graphics(paste0(".", "/images/", "F67P02", "-Default-1x-Box", ".png"))
```
...
- \textcolor{pink}{glm()} 
  ...
  - A 'date' is a numeric variable and thus is treated as a numerical in logit model. It needs to be converted to Factor, Year, Month, Day, Week Number, Weekday etc. for usage.
- Dataset Default - 
  - Y ~ Balance
...
- Class-specific performance: The terms sensitivity and specificity characterize the performance of a classifier or screening test. 
  - Depending upon the Factor level that "Event" corresponds to a "positive" result 
    - Event: Default = "Yes"
  - \textcolor{pink}{No Information Rate (NIR)} is the largest class percentage in the data.
    - i.e. proportion of majority class in the original dataset
    - We want Accuracy to be greater than NIR
    - Further, we want to know if the accuracy being higher than NIR is significant. A test of whether an observed proportion is greater than some specified value is a simple one-tailed binomial test. The p-value from which is what the \textcolor{pink}{P-Value [Acc > NIR]} displays. 
    - However, the p-value only tells that the accuracy is better, we may also want to know if our model is biased i.e., does our classifier tend to say a class more often than is really the case.
  - \textcolor{pink}{Sensitivity or Recall or Power or True Positive Rate}, is the percentage of true defaulters that are identified
    - $\text{Sensitivity} = \text{Recall} = \frac{\text{TP}}{\text{TP} + \text{FN}} = \frac{A}{A + C} = \frac{79}{79 + 254} = \frac{79}{333} = 0.2372372 = 1 - \text{Type-II Error}$
      ...
      - Recall is useful to make judgements about the model and we want to see how much the model is able to recall correctly
  ...
  - \textcolor{pink}{Positive Predictive Value (PPV) or Precision} 
    - $\text{PPV} = \frac{\text{Sensitivity} \times \text{Prevalence}}{(\text{Sensitivity} \times \text{Prevalence}) + ((1 - \text{Specificity}) \times (1 - \text{Prevalence}))} = 0.782177$
    - `{0.2372372 * 0.0333 / (0.2372372 * 0.0333 + ((1 - 0.9977242) * (1 - 0.0333)))}` \textcolor{pink}{$\#\mathcal{R}$}
    - $\text{PPV} = 1 - \text{Sensitivity} = \frac{C}{A+C}$
    ...
    - Precision is useful in actual targeting plan; we can only act on predicted and so this is useful
    - It is also same as \textcolor{pink}{(1 - False Discovery Proportion)}
  - \textcolor{pink}{Negative Predictive Value (NPV)} 
    - $\text{NPV} = \frac{\text{Specificity} \times (1 - \text{Prevalence})}{((1 - \text{Sensitivity}) \times \text{Prevalence}) + ((\text{Specificity}) \times (1 - \text{Prevalence}))} = 0.9743408$    
  ...
  - \textcolor{pink}{Detection Rate} 
    - $\text{Detection Rate} = \frac{\text{TP}}{\text{Total}} = \frac{A}{N} = \frac{79}{10000} = 0.0079$
  ...
  - \textcolor{pink}{Accuracy or Overall Accuracy} 
    - $\text{Accuracy} = \frac{\text{TP} + \text{TN}}{\text{Total}} = \frac{A + D}{N} = \frac{79+9645}{10000} = 0.9724$
...
- \textcolor{pink}{McNemar test}
  - The p-value only tells that the accuracy is better, we may also want to know if our model is biased i.e., does our classifier tend to say a class more often than is really the case.
  - We can test the proportion the model says B against the observed proportion B, but that would not really take the structure of the confusion matrix into account. It would not recognize that the underlying data are non-independent, and therefore does not deal with the non-independence appropriately.     - The test that does recognize these things is McNemar test. 
  - McNemar Test captures the errors made by both models. Specifically, the No/Yes and Yes/No cells in the confusion matrix. The test checks if there is a significant difference between the counts in these two cells. 
  ...
  - The null hypothesis is more or less that the proportions of one variable are equal across both values of the other variable. 
      - A significant test result means that you have rejected this null hypothesis, and decided that your two variables are associated (i.e. that knowing something about one, gives you information about the other), and therefore that the proportion of one variable changes depending on the values of the other variable.
  - It is important to know that there are two different scenarios in the context of machine learning.
    - The first one is that you are evaluating the quality of your model vs. the reference data (test data or possibly training data). 
    - The second scenario is when you are comparing two classification models (algorithms).
  - In the first scenario you would normally look for the p-value of the test to be greater than 0.05.
    - i.e., do not reject the null hypothesis that assumes homogeneity of the proportion of misclassified cases for the two class labels. A significant value here would indicate that your algorithm misclassifies one label more than another.
  - In the second scenario you would probably look for the opposite. i.e., that the p-value of the test is less than 0.05. 
    - This would indicate that the classifiers have different error rates. This is evidence that you can use in conjunction with other indicators such as individual model accuracy to conclude that one is better than another.
...
```{definition 'ROC'}
\textcolor{pink}{ROC curve}, or Receiver Operating Characteristics, simultaneously display the two types of errors (Sensitivity (TPR) vs. {1 - Specificity} (FPR)) for all possible threshold values for the posterior probability of the Event. The overall performance of a classifier, summarized over all possible thresholds, is given by the area under the (ROC) curve \textcolor{pink}{(AUC)}. An ideal ROC curve will hug the top left corner, so the larger area under the (ROC) curve the AUC the better the classifier. 
```


- The \textcolor{pink}{ROC curve}
  - Y: The true positive rate (TPR) is the sensitivity: the fraction of defaulters that are correctly identified, using a given threshold value. 
  - X: The alse positive rate (FPR) is 1-specificity: the fraction of non-defaulters that we classify incorrectly as defaulters, using that same threshold value. 
...
- The Error & Warning occur if posterior probability matrix is passed. Its subset in the form of vector is required.
- Also applicable for logistic regression using rpart
...
include_graphics(paste0(".", "/images/", "F67P03", "-Default-ROC-LDA", ".png"))
```
