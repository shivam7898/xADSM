...
tibble(Names = names(cmat$byClass), 
...
#
# #Confusion Matrix
if(TRUE) cmat_def_lda_50$table %>% as_tibble() %>% pivot_wider(names_from = Reference, values_from = n) %>% 
  rename(Prediction_Reference = 1) %>% 
  add_row(summarise(., across(1, ~"Total")), summarise(., across(where(is.numeric), sum))) %>% 
  mutate(SUM = rowSums(across(where(is.numeric))))
...
#
# #Confusion Matrix
if(TRUE) cmat_def_lda_80$table %>% as_tibble() %>% pivot_wider(names_from = Reference, values_from = n) %>% 
  rename(Prediction_Reference = 1) %>% 
  add_row(summarise(., across(1, ~"Total")), summarise(., across(where(is.numeric), sum))) %>% 
  mutate(SUM = rowSums(across(where(is.numeric))))
#
...
tibble(Names = names(cmat_def_lda_50$byClass), 
...
if(TRUE) cmat$table %>% as_tibble() %>% pivot_wider(names_from = Reference, values_from = n) %>% 
  ...
  mutate(SUM = rowSums(across(where(is.numeric))))
#
# #Compare Accuracy: LDA vs. QDA
if(TRUE) cmat_def_lda$overall[["Accuracy"]]
if(TRUE) cmat_def_qda$overall[["Accuracy"]]
#
# #Class Comparisons: LDA vs. QDA
tibble(Names = names(cmat_def_qda$byClass), 
     LDA = round(cmat_def_lda$byClass, 4), QDA = round(cmat_def_qda$byClass, 4),
     Change = QDA - LDA)
```
