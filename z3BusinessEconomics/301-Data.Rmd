...
```{r 'C01P01-Save', include=TRUE}
# #Save an Image
ggsave(paste0(.z$PX, "C01P01_Cars_TimeSeries", ".png"), plot = C01P01)
```

```{r 'C01P01', echo=TRUE, fig.cap="Multiple Time Series Graph"}
# #Load an Image
...
```{r 'C01-K-Pareto-A', ref.label=c('C01-K-Pareto'), eval=FALSE}
# #Display Code without showing the associated Graph
```

```{r 'C01P02-Save', include=FALSE}
# #Save without using ggsave()
png(filename = paste0(.z$PX, "C01P02_Cars_Pareto", ".png"))
plot(C01P02)
dev.off()
```

```{r 'C01P02', echo=FALSE, fig.cap="Pareto of Cars in 2010"}
knitr::include_graphics(paste0(.z$PX, "C01P02_Cars_Pareto", ".png"))
```
