...
- Exercises: Mean, Percentage, Transpose, Rowwise Calculations
- Graphs: Multiple Time Series, Pareto Chart
...
\textcolor{pink}{Statistics} is the art and science of collecting, analyzing, presenting, and interpreting data. 
```
...
> The measurable quality /characteristic is called a "Parameter" for Population and "Statistic" for Sample.
...
- Table: \@ref(tab:301DI-xxComputers) 
- Table: \@ref(tab:301DJ-xxCordless)  
- Table: \@ref(tab:301DK-CarsMultiple)  
...
```{r '301F-f_getObject', ref.label=c('A07-getObject'), eval=FALSE}
# #
```
...
```{r '301CI-Assignment', include=FALSE}
bb <- xxComputers
```

```{r '301DI-xxComputers', echo=FALSE}
bb <- xxComputers
# #Kable Table
kbl(bb,
  caption = 'xxComputers',
...
```{r '301CJ-Assignment', include=FALSE}
bb <- xxCordless
```

```{r '301DJ-xxCordless', echo=FALSE}
bb <- xxCordless
# #Kable Table
kbl(bb,
  caption = 'xxCordless',
...
### Cars {.unlisted .unnumbered .tabset .tabset-fade} 

```{r '301CK-Assignment', include=FALSE}
bb <- tibble(Company = c("Hertz", "Dollar", "Avis"), 
              `2007` = c(327, 167, 204), `2008` = c(311, 140, 220),
              `2009` = c(286, 106, 300), `2010` = c(290, 108, 270))
# #Transpose Tibble: Note that the First Column Header is lost after Transpose
...
```{r '301DK-CarsMultiple', echo=FALSE}
kk_bb <- kbl(bb,
...
```{r '301KA-Transform', ref.label=c('301CK-Assignment'), eval=FALSE}
# #
```

#### TimeSeries {.unlisted .unnumbered}
```{r '301KD-TimeSeries', include=FALSE}
...
```{r '301KG-Pareto-A', ref.label=c('301KG-Pareto'), eval=FALSE}
# #Display Code without showing the associated Graph
```

```{r '301KH-Save', include=FALSE}
# #Save
