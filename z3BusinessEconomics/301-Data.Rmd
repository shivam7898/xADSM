# Data and Statistics (BE01) {#be01}
> Definitions and Exercises are from the Book [@Anderson]

```{r '301A', include=FALSE, cache=FALSE}
...
- This chapter covers introductory information and basic definitions.
  - [Exercises](#exercises-be01 "be01"): Mean, Percentage, Transpose, Rowwise Calculations
  - Graphs: [Multiple Time Series](#cars-be01 "be01"), [Pareto Chart](#cars-be01 "be01")
...
See Table \@ref(tab:vs-interval-ratio) for more details.
...
Also, "0" on ratio scale means the absence of that physical quantity. "0" on interval scale doesn't mean the same. 0 kg means the absence of weight. 0 °C doesn't mean absence of heat.

Table: (\#tab:vs-interval-ratio) Interval scale Vs Ratio scale
...
A sample statistic is referred to as the \textcolor{pink}{point estimator} of the corresponding population parameter. For example, $\overline{x}, s, s^2, s_{xy}, r_{xy}$ sample statics are point estimators for corresponding population parameters of $\mu$ (mean), $\sigma$ (standard deviation), $\sigma^2$ (variance), $\sigma_{xy}$ (covariance), $\rho_{xy}$ (correlation).
```

## Difference between a population and a sample {#PopulationSample}
...
A \textcolor{pink}{random sample} is a sample that is chosen randomly. It could be more accurately called a 'randomly chosen sample'. Random samples are used to avoid bias and other unwanted effects.
```

```{definition 'SRS'}
A \textcolor{pink}{simple random sample (SRS)} is a set of $n$ objects in a population of $N$ objects where all possible samples are equally likely to happen.
```
...
## Exercises {#exercises-be01}

- Table: \@ref(tab:301-I-xxComputers) 
- Table: \@ref(tab:301-J-xxCordless)  
- Table: \@ref(tab:301-K-CarsMultiple)  
...
```{r '301-Load'}
xxComputers <- f_getObject("xxComputers", "BE01-Computers.csv", "971fb6096e4f71e8185d3327a9033a10")
xxCordless <- f_getObject("xxCordless", "BE01-Cordless.csv", "9991f612fe44f1c890440bd238084679")
```

#### f_getObject() {.unlisted .unnumbered}
```{r '301F-getObject', ref.label=c('A07-getObject'), eval=FALSE}
# #
```

#### Print Table {.unlisted .unnumbered}

```{r '301D-KBL', eval=FALSE}
bb <- xxComputers
#displ_names <- c("") 
...
```{r '301-I-Assignment', include=FALSE}
bb <- xxComputers
```

```{r '301-I-xxComputers', echo=FALSE}
bb <- xxComputers
#displ_names <- c("") 
#stopifnot(identical(ncol(bb), length(displ_names)))
#
...
```{r '301-J-Assignment', include=FALSE}
bb <- xxCordless
```

```{r '301-J-xxCordless', echo=FALSE}
bb <- xxCordless
#displ_names <- c("") 
#stopifnot(identical(ncol(bb), length(displ_names)))
#
...
### Cars {#cars-be01 .unlisted .unnumbered .tabset .tabset-fade} 

```{r '301-K-Assignment', include=FALSE}
bb <- tibble(Company = c("Hertz", "Dollar", "Avis"), 
              `2007` = c(327, 167, 204), `2008` = c(311, 140, 220),
              `2009` = c(286, 106, 300), `2010` = c(290, 108, 270))
# #Transpose Tibble: Note that the First Column Header is lost after Transpose
...
```{r '301-K-CarsMultiple', echo=FALSE}
#displ_names <- c("") 
#stopifnot(identical(ncol(bb), length(displ_names)))
#
kk_bb <- kbl(bb,
  caption = 'Cars in Service (1000s)',
  #col.names = displ_names,
  escape = FALSE, align = "c", booktabs = TRUE
  ) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),
...
```{r '301-K-Transform', ref.label=c('301-K-Assignment'), eval=FALSE}
# #
```

#### TimeSeries {.unlisted .unnumbered}

```{r '301-K-TimeSeries', include=FALSE}
# #Time series graph for all companies on the same graph
g301_01 <- ggplot(data = hh, 
                     aes(x = Year, y = Values, colour = Company, group = Company)) + 
  geom_line()
```

```{r '301-K-SaveTimeSeries', include=TRUE}
# #Save
ggsave(paste0(.z$PX, "g301_Cars_01_TimeSeries", ".png"), plot = g301_01)
```

```{r '301G-Cars01-TimeSeries', echo=TRUE, fig.cap="Multiple Time Series Graph"}
# #Import
knitr::include_graphics(paste0(.z$PX, "g301_Cars_01_TimeSeries", ".png"))
```
...
# #Plot Pareto
g301_02 <- pareto.chart(pareto_chr, 
             xlab = "Company",
			 ylab = "Cars",
             # colors of the chart             
...
```{r '301-K-Pareto-A', ref.label=c('301-K-Pareto'), eval=FALSE}
# #Display Code without showing the associated Graph
```

```{r '301-K-SavePareto', include=FALSE}
# #Save
png(filename = paste0(.z$PX, "g301_Cars_02_Pareto", ".png"))
plot(g301_02)
dev.off()
```

```{r '301G-Cars02-Pareto', echo=FALSE, fig.cap="Pareto of Cars in 2010"}
# #Import
knitr::include_graphics(paste0(.z$PX, "g301_Cars_02_Pareto", ".png"))
```

## Validation {.unlisted .unnumbered .tabset .tabset-fade}

```{r '301-Cleanup', include=FALSE, cache=FALSE}
f_rmExist(aa, bb, g301_01, g301_02, hh, ii, jj, kk, kk_bb, kk_hh, kk_ii, pareto_chr)
f_rmExist(xxComputers, xxCordless)
```

```{r '301-Validation', include=FALSE, cache=FALSE}
