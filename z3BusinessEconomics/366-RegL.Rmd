...
- About: [200, 4]
  ...
  - It has sales of product in 200 different markets, along with advertising budgets for the product in each of those markets for three different media: TV, radio, and newspaper.
    - Advertising Budgets for each media is in USD 1000 
...
- [Dummies should be converted Explicitly](#exp-dummy-f66 "f66")
  - By default, lm() can convert dummies but it is better to explicitly handle those. - "ForLater"
...
- [Variables like Age or Education Levels should be treated as Continuous](#age-con-f66 "f66")
  ...
  - Marks given to Students (e.g. 44 or 63 out of 100, 3 or 4 out of 10) although look continuous but inherently these are categorical. However, almost always, they are assumed to be continuous to calculate a Grade point average (GPA) etc. 
    - \textcolor{orange}{Caution:} According to Statistical theory this is wrong but it happens this way only.
  - As the number of levels increases, we tend to assume that the variable is continuous. So, there is no set boundary when a variable is categorical or continuous
  - Treating them as continuous, automatically handles the missing levels that are not in the data i.e. age (e.g. 40, 55 might be missing). Converting this to categorical would create another problem of how to treat those.
    - Distance between 39 and 41 would change with the presence or absence of an observation value of 40 if we treat it as categorical.
...
- Ordered Factors are converted into polynomial by default in R during regression whereas unordered factors are kept simple
...
- \textcolor{pink}{Question:} Refer [Answers](#marketing-f66 "f66")
...
- Similarly standard errors for $\beta_0, \beta_1$ can be estimated
  ...
  - In general, $\sigma^2$ is not known, but can be estimated from the data. This estimate of $\sigma$ is known as the RSE. 
    - Definition given below is applicable similarly to the estimation of model coefficients. 
...
- p-value
  ...
  - We reject the null hypothesis —that is, we declare a relationship to exist between X and Y —if the p-value is small enough. 
    - Typical p-value cut-offs for rejecting the null hypothesis are 5% or 1%. 
...
- \textcolor{pink}{lm()} for Linear Regression
  ...
  - \textcolor{pink}{predict()} for prediction i.e. $\hat{y}$ 
	...
	- If no dataset is supplied, then the training data is used for model prediction
	- predict.rpart() : type = c("vector", "prob", "class", "matrix")
	- predict.glm() : type = c("link", "response", "terms")
	- predict.lm() : type = c("response", "terms")
	- predict.randomForest() : type = c("response")
	- predict.train() : type = c("raw", "prob")
...
- Dataset Advertising - Linear Regression of Y (Sales) vs. Single X (TV)
  ...
  - According to this approximation, an additional unit (USD 1000) spent on TV advertising is associated with selling approximately 0.0475 additional units of the product (in 1000 i.e. 47.5 units)
  - Confidence Interval
    ...
    - $\beta_1 \in [0.042, 0.053]$ : For each USD 1000 increase in television advertising, there will be an average increase in sales of between 42 and 53 units
  - Hypothesis Test
...
- Dataset Advertising - Linear Regression of Y (Sales)
  - For a given amount of TV and newspaper advertising, spending an additional USD 1000 on radio advertising is associated with approximately 188 units of additional sales. 
  ...
  - This illustrates that the simple and multiple regression coefficients can be quite different. 
    - This difference stems from the fact that in the simple regression case, the slope term represents the average increase in product sales associated with a USD 1000 increase in newspaper advertising, \textcolor{pink}{ignoring other predictors} such as TV and radio. 
    - By contrast, in the multiple regression setting, the coefficient for newspaper represents the average increase in product sales associated with increasing newspaper spending by USD 1000 while \textcolor{pink}{keeping other predictors fixed}.
...
include_graphics(paste0(".", "/images/", "F66P08", "-Ads-Corrplot", ".png"))
```
...
- We should look at \textcolor{pink}{Adjusted R Square $R_a^2$}
  ...
  - Model with 2 variables has higher (better) $R_a^2$ and thus should be used
...
- Dataset Advertising - Linear Regression of Y (Sales)
  - We use a confidence interval to quantify the uncertainty surrounding confidence interval the average sales over a large number of cities. 
    - Ex: Given that USD 100,000 are spent on TV advertising and USD 20,000 are spent on radio advertising in each city, the 95% confidence interval is [10985, 11528]. 
  ...
  - On the other hand, a prediction interval can be used to quantify the prediction interval uncertainty surrounding sales for a particular city. 
    - Ex: Given that USD 100,000 are spent on TV advertising and USD 20,000 are spent on radio advertising in that city the 95% prediction interval is [7930, 14583]. 
...
- Dataset Credit - Linear Regression of Y (Balance) vs. Single Categorical X (Own)
  ...
  - The average credit card debt for non-owners is estimated to be USD 509.80 (Constant), whereas owners are estimated to carry USD 19.73 in additional debt for a total of USD 529.53. 
  ...
  - Alternate Reference: 1 (does not own house), 0 (owns the house)
    - The average credit card debt for owners is estimated to be USD 529.53 (Constant), whereas non-owners are estimated to carry slightly lower debt of USD 19.73 for a total of USD 509.80. 
  - Alternate Reference: -1 (does not own house), 1 (owns the house)
    - The average credit card debt for owners is estimated to be USD 519.67 (Constant)
    - Non-owners are estimated to carry slightly lower debt of USD (-1) * 9.87 for a total of USD 509.80. 
    - Owners are estimated to carry additional debt of USD (1) * 9.87 for a total of USD 529.53.
...
- Dataset Credit - Linear Regression of Y (Balance) vs. Single Categorical X (Region)
  ...
  - \textcolor{orange}{Caution:} Book might have interchanged the Coefficients of Region_South and Region_West
    - The estimated balance for the baseline, East, is USD 531. 
    - It is estimated that those in the South will have USD 12 less debt than those in the East
    - Those in the West will have USD 18 less debt than those in the East. 
...
- In our previous analysis of the Advertising data, we concluded that both TV and radio seem to be associated with sales. 
  ...
  - However, this simple model may be incorrect. 
    ...
    - In this situation, given a fixed budget of USD 100,000, spending half on radio and half on TV may increase sales more than allocating the entire amount to either TV or to radio. 
...
- Dataset Advertising - Linear Regression of Y (Sales)
  ...
  - Coefficients
    - An increase in TV advertising of USD 1000 is associated with increased sales of $(\hat{\beta}_1 + \hat{\beta}_3 \times \text{Radio}) \times 1000 = 19 + 1.1 \times \text{Radio}$ units.
    - An increase in Radio advertising of USD 1000 will be associated with an increase in sales of $(\hat{\beta}_2 + \hat{\beta}_3 \times \text{TV}) \times 1000 = 29 + 1.1 \times \text{TV}$ units.
...
sub_hh <- "Large residuals at edges, Small at Center: Non-linearity!"
```
...
include_graphics(paste0(".", "/images/", "F66P09", "-Auto-ResidFit-1x", ".png"))
```
...
include_graphics(paste0(".", "/images/", "F66P10", "-Auto-ResidFit-2x", ".png"))
```
...
- Multicollinearity is difficult to detect by inspection of the correlation matrix.
  ...
  - There are two solutions
    - Either drop one of the multicollinear variable because the information it provides about the Response is redundant in the presence of other variable
...
include_graphics(paste0(".", "/images/", "F66P11", "-Credit-Corrplot", ".png"))
```
...
## The Marketing Plan {#marketing-f66}
...
4. How large is the association between each medium and sales
    ...
    - 95% confidence intervals for the coefficients: 
      ...
      - Best Model: (0.016, 0.022) for TV, (0.011, 0.046) for radio, and (0.001, 0.001) for interaction between TV and Radio. 
...
- Note: 
  ...
  - Thus, in a real life situation in which the true relationship is unknown, one might suspect that KNN should be favoured over linear regression because it will at worst be slightly inferior to linear regression if the true relationship is linear, and may give substantially better results if the true relationship is non-linear. 
...
include_graphics(paste0(".", "/images/", "F66P01", "-Boston-lm-1x-lstat", ".png"))
```
...
include_graphics(paste0(".", "/images/", "F66P02", "-Boston-1x-YX", ".png"))
```
...
include_graphics(paste0(".", "/images/", "F66P03", "-Boston-YX", ".png"))
```
...
- The Error & Warning occur if model result is passed to tidy() in place of actual model
...
include_graphics(paste0(".", "/images/", "F66P04", "-Boston-1x-Resid-Fit", ".png"))
```
...
include_graphics(paste0(".", "/images/", "F66P05", "-Boston-lm-QQ", ".png"))
```
...
include_graphics(paste0(".", "/images/", "F66P06", "-Boston-lm-Scale-Loc", ".png"))
```
...
include_graphics(paste0(".", "/images/", "F66P07", "-Boston-lm-Leverage", ".png"))
```
...
# #Linear Regression with Continuous & Categorical X
# #Dummy |
zzF66Credit_dum <- dummy_cols(zzF66Credit, 
                              remove_first_dummy = TRUE, remove_selected_columns = TRUE)
#
...
#
# #Predicted /Calculated Response Y i.e. hat{Y}
...
f_rmExist(aa, bb, ii, jj, kk, ll, cap_hh, col_hh, corr_hh, F66P02, F66P03, F66P04, F66P05, F66P06,
          ...
          zzF66CarSeats, zzF66CarSeats_dum, zzF66Credit, zzF66Credit_dum, F66P01, F66P08, F66P11)
```
