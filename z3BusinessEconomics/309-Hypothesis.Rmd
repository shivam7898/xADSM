...
\textcolor{pink}{$\text{\{Left Tail or Lower Tail\} } {H_0} : {\mu} \geq {\mu}_0 \iff {H_a}: {\mu} < {\mu}_0$}
```

```{definition 'Hypothesis-1T-Upper-Tail'}
\textcolor{pink}{$\text{\{Right Tail or Upper Tail\} } {H_0} : {\mu} \leq {\mu}_0 \iff {H_a}: {\mu} > {\mu}_0$}
```

```{definition 'Hypothesis-2T-Two-Tail'}
\textcolor{pink}{$\text{\{Two Tail\} } {H_0} :{\mu} = {\mu}_0 \iff {H_a}: {\mu} \neq {\mu}_0$}
```
