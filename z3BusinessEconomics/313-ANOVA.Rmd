...
- \@ref(eq:f-anv) $F = \frac{260}{28.33} = 9.18$
- Numerator : $\text{DOF} = k - 1 = 3 - 1 = 2$
- Denominator : $\text{DOF} = {n}_T - k = 15 - 3 = 12$
...
- Compare with ${\alpha} = 0.05$
  - ${}^U\!P_{(F)} < {\alpha} \to {H_0}$ is rejected i.e. 
...
summary(bb_anova)
#
# #Df shows the degrees of freedom for the independent variable (levels - 1)
# #and the degrees of freedom for the residuals (total observations - 1 - levels).
# #Sum Sq shows the sum of squares (total variation between the group means and the overall mean)
# #Mean Sq shows the mean of the sum of squares i.e. 
# #(sum of squares / degrees of freedom for each parameter).
# #F-value column is the test statistic from the F test. 
# #i.e. Mean square of each independent variable / mean square of the residuals. 
# #The larger the F value, the more likely it is that 
...
# #i.e. likelihood that the F-value calculated from the test would have occurred 
# #if the null hypothesis of no difference among group means were true.
```
