...
tbl <- read_csv(paste0(.z$XL, "C34-adult.csv"),
...
```{r 'C34-PartAdult'}
#wwww
# #Select Specific Variables (BOOK)
bb <- xxC34Adult %>% 
  mutate(capnet = capital_gain - capital_loss) %>% 
  select(age, fnlwgt, education_num, hours_per_week, capnet)
set.seed(3)  
...
#
# #Training Data
adl_xw <- part_l$train %>% rename(edu = 3, hours = 4) 
# #Scaling
adl_zw <- adl_xw %>% mutate(across(everything(), ~ as.vector(scale(.))))
# #Long
f_wl(adl_xw, adl_zw)
```
...
f_rmExist(aa, bb, ii, jj, kk, ll, c32churn, xxB16Cars, xxB18Churn, brk_bb, hh, idx_bb, part_l,
          ...
          eigen_pca, load_pca, psy_zw, rec_ii, rec_jj, sd_pca, swp_ii, swp_jj, vv)
```
