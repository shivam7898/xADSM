...
if(FALSE) paste0(names(bb), collapse = ", ")
names_bb <- c("h_value", "income", "h_age", "rooms", "bedrooms", 
               "population", "households", "latitude", "longitude")
names(names_bb) <- c("House Value (Median)", "Income (Median)", "House Age (Median)", 
...
xsyl <- bb %>%  
  ...
  mutate(across(Keys, factor, levels = unique(Keys)))
#
# #Summary
...
levels(xsyg$Keys)
levels(xsyg$Keys) <- names(names_bb)
levels(xsyg$Keys)
```
...
#
cap_hh <- paste0("(C34T01)", "[", nrow(bb), ", ", ncol(bb), "] ", "Houses: Basics") 
...
bb <- part_l$train
#
zsyl <- bb %>% mutate(across(everything(), ~ as.vector(scale(.)))) %>% 
  pivot_longer(everything(), names_to = "Keys", values_to = "Values") %>% 
  mutate(across(Keys, factor, levels = unique(Keys), labels = names(names_bb)))
```
...
zsyw <- zsyl %>% 
  ...
  select(-1) %>% 
  rename(Target = 1, Income = "Income (Median)", Age = "House Age (Median)", 
         Rooms = "Rooms (Total)", Beds = "Bedrooms (Total)", Pop = "Population", 
         Houses = "Households", Lat = "Latitude", Long = "Longitude") #%>% 
  #pivot_longer(-c(1), names_to = "Keys", values_to = "Values") %>% 
  #mutate(across(Keys, factor, levels = unique(Keys), labels = names(names_bb)[-1]))
#
hh <- zsyw
#hh <- hh[1:100, ]
#
...
C34 <- hh %>% { 
  ggpairs(data = ., mapping = aes(alpha = I(0.1)), 
          columns = 2:ncol(.), 
...
if(!file.exists(loc_png)) {
  ...
  corrplot(cor(hh[2:ncol(hh)]), method = "circle", type = "full", diag = FALSE, 
...
```{r 'C34P03', echo=FALSE, fig.cap="Houses: Corrplot (8)", out.width='100%'}
knitr::include_graphics(paste0(.z$PX, "C34P03", "-House-Corplot-z", ".png")) #iiii
```
...
# #Print Kable Table
hh <- cor(zsyw[2:ncol(zsyw)])
cap_hh <- paste0("(C34T02) ", "Houses: Correlation Matrix") 
f_pKblM(x = hh, caption = cap_hh)
```
