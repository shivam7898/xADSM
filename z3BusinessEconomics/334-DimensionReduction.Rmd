...
invisible(lapply(f_getPathR(A12pKbl), knitr::read_chunk))
```
...
include_graphics(paste0(".", "/images/", "C34P09", "-Multicollinearity-zz", ".png")) #iiii
```
...
Let \textcolor{pink}{$\mathbf{B}$} be an $m \times m$ matrix, and let ${\lambda}$ be an eigenvalue of $\mathbf{B}$. Then nonzero $m \times 1$ vector \textcolor{pink}{$\overrightarrow{e}$} is said to be an \textcolor{pink}{eigenvector of B}, if $\mathbf{B} \overrightarrow{e} = \lambda \overrightarrow{e}$.
```

- The total variability in the standardized set of predictors equals the sum of the variances of the Z-vectors, which equals the sum of the variances of the components, which equals the sum of the eigenvalues, which equals the number of predictors
...
include_graphics(paste0(".", "/images/", "C34P01", "-Houses-Box-Z", ".png"))
```
...
include_graphics(paste0(".", "/images/", "C34P02", "-House-SPLOM-z", ".png"))
```
...
include_graphics(paste0(".", "/images/", "C34P03", "-House-Corplot-z", ".png"))
```
...
- \textcolor{pink}{cor()} : Correlation Function produces a Matrix
  - Matrices has HUGE number of problems but unfortunately some function output is in that form
  - names() does NOT work on Matrices but colnames() works, even though names() is superior to colnames() in all other aspects
  - Symmetrical Matrix. Diagonal and one of the Triangles (Upper or Lower) are redundant
...
- Understanding the result
  ...
  - Each of the columns in Table represents one of the components $Y_i = e_i^T\mathbf{Z}$.
...
include_graphics(paste0(".", "/images/", "C34P04", "-House-Corplot-PCA", ".png"))
```
...
include_graphics(paste0(".", "/images/", "C34P05", "-House-PCA-Scree", ".png"))
```
...
include_graphics(paste0(".", "/images/", "C34P06", "-House-PCA-Var", ".png"))
```
...
- The Proportion of Variance Explained Criterion
  - We can define how much of the total variability that we would like the principal components to account for and then selects them accordingly
...
#
cap_hh <- paste0("(C34T05) ", "Houses: PCA Eigenvectors till PC4") 
...
include_graphics(paste0(".", "/images/", "C34P07", "-House-SPLOM-PC3", ".png"))
```
...
include_graphics(paste0(".", "/images/", "C34P08", "-House-SPLOM-PC4", ".png"))
```
...
- Thus, the Table \@ref(tab:C34T03) is modified further as \@ref(tab:C34T06) 
  ...
  - NOTE: Values should be matching when eigenvectors are converted to loadings
...
rownames(hh) <- names(c_zsyw)[-1]
#
cap_hh <- paste0("(C34T06) ", "Houses: PCA Loadings (not Eigenvectors) till PC4") 
f_pKblM(x = hh, caption = cap_hh, isTri = FALSE, dig = 3, debug = FALSE)
```
...
matrix(1:9, nrow = 3)
#
# #Change Behaviour to fill the matrix by Row
...
- Test Dataset
  ...
  - If the results differ too much then it should be taken as the indication of training data being not a representation of test data. 
...
- Repository of many standard datasets
  - Archived: https://archive.ics.uci.edu/ml/datasets.php
...
- About: Train [32561, 15] & Test[16281, 15] = Total[48842, 15]
  - The intended task is to find the set of demographic characteristics that can best predict whether or not the individual has an income of over USD 50000 per year. 
- Steps [(External) Luke Perich](https://rpubs.com/s3589539/503258 "https://rpubs.com")
  ...
  - 'capital_gain' & 'capital_loss' - Both removed
    - After USD 41310, There is directly USD 99999 which cannot be correct. It should be set to Median if it is kept
...
bb <- aa <- xxC34Adult <- f_getRDS(xxC34Adult)
```
...
### Merge Factor Levels {#ref-merge-factor-c34 .unlisted .unnumbered}

```{r 'C34-Merge-Factors', eval=FALSE}
...
bb$x
#
# #Alphabetical sorting by default, use fct_inorder() for 'in order of appearance' 
ii <- bb %>% mutate(x = fct_collapse(fct_inorder(x), No = c("No", "N"), Yes = c("Yes", "Y"), NULL = c("H")))
#
# #Alphabetical sorting by default, use fct_inorder() for 'in order of appearance' 
jj <- bb %>% mutate(x = fct_recode(fct_inorder(x), Yes = "Y", No = "N", NULL = "H"))
#
# #As provided in levels, Not Alphabetical #Current in use
kk <- bb %>% mutate(across(x, factor, levels = c("Y", "Yes", "N", "No"), 
                           labels = c("Yes", "Yes", "No", "No"))) 
#
# #As provided in List, Not Alphabetical
ll <- bb
levels(ll$x) <- list(Yes = c("Yes", "Y"), No = c("No", "N"))
#
# #Modify only the relevant levels. However, due to the use of indexing it might be error prone
# #Levels are as given in original (alphabetical), would require another step to be identical
mm <- bb
levels(mm$x)[c(1, 2, 4)] <- c(NA, "No", "Yes")
mm$x
#
...
### Replace Multiple Partial Matches {.unlisted .unnumbered}
...
NOTE: While the Book created 'Net Capital', I have skipped that because Capital Gain and Capital Loss Columns have extremely high number of zeroes. Further, an ID column 'fnlwgt' was also dropped.
...
- \textcolor{orange}{Warning:} The estimated weights for the factor scores are probably incorrect.  Try a different factor score estimation method.
  - It is coming most probably because age and education might be serially correlated i.e. 
...
- NOTE
  - Cumulative Variance is only 48% i.e. less than half is explained by first two factors extracted.
